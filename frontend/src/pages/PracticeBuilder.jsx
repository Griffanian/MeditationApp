...
export default function PracticeBuilder() {
  ...
  useEffect(() => {
    function handleMutations(e) {
      ...
      if (mutations.items) {
        setPractice(prev => ({ ...prev, items: migrateToWeeks(mutations.items) }));
      }
    }
    window.addEventListener('chat-mutations', handleMutations);
    return () => window.removeEventListener('chat-mutations', handleMutations);
  }, []);
